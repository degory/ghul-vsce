...
compiler's analysis-mode protocol over IPC, so it is coupled to `ghul.compiler`:
...
The extension is coupled to the compiler's analysis-mode protocol: a breaking
protocol change is a major here as well as there. Major means a removed or
incompatible protocol message, or a change requiring a compiler users do not yet
have; minor means new commands, settings or protocol messages the compiler can
ignore.
